	...
	, _currentPresetIdx(0)
{
    ...
    // Check if magic.xml exists, if not create an empty one
    if (::BinaryData::magic_xmlSize == 0)
    {
        // Create an empty magic.xml file
        juce::File magicFile = juce::File(MY_MAGIC_SRC);
        magicFile.replaceWithText("");
        
        // Don't load anything - let the GUI build itself
    }
    else
    {
        // Try to load the magic.xml and validate it
        try 
        {
            auto xml = juce::parseXML(juce::String::createStringFromData(::BinaryData::magic_xml, ::BinaryData::magic_xmlSize));
            
            if (xml != nullptr && xml->hasTagName("magic"))
            {
                // XML is valid, load it
                magicState.setGuiValueTree (::BinaryData::magic_xml, ::BinaryData::magic_xmlSize);
            }
            else
            {
                // XML is invalid or doesn't match expected structure - skip loading
                DBG("magic.xml is invalid or has wrong root tag - using empty GUI state");
            }
        }
        catch (...)
        {
            // Error parsing XML - skip loading
            DBG("Error parsing magic.xml - using empty GUI state");
        }
    }
